...
class TelaSalaProva(FaseBase):
    ...
    def _inicializar_questoes(self):
        """Sorteia a prova a partir do banco de perguntas (data/questions.json).

        5 fáceis + 5 médias + 5 difíceis (15 no total), sorteadas sem
        repetição a cada prova. A ordem final continua fácil -> média ->
        difícil, mantendo a progressão de dificuldade prevista no README.
        """
        self.questoes = sortear_prova(qtd_facil=5, qtd_media=5, qtd_dificil=5)
